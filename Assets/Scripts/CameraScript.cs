...
public class CameraScript : MonoBehaviour {
	...
	void Update () {
		...
		if(Input.GetMouseButtonDown(0) && Input.mousePosition.y > 200) {
			...
			if(!GameObject.Find("GameController").GetComponent<GameController>().isActionButton) {
				...
				if (tile.gameObject.activeSelf) {
					...
					if(tile.GetComponent<MeshRenderer>().sharedMaterial.color == Color.green) {
						...
						if(buildPlayer != null) {
							...
							if(player.AddBuilding (GameController.map, buildPlayer, new Vector2 (canBuildColumn, canBuildLine))) {
                                ...
                                foreach(Unit unit in GameObject.Find("GameController").GetComponent<GameController>().selectedUnits) {
								...
								/**
								foreach(Worker worker in GameObject.Find("GameController").GetComponent<GameController>().selectedUnits) {
									worker.targetBuilding = buildInstance;
									worker.isWalking = false;
								}
								/**/

								GameObject.Find("GameController").GetComponent<GameController>().LeftClick (GameObject.Find("GameController").GetComponent<GameController>().selectedUnits[0].model.name, hit.transform.position);
					...
					if(hit.transform != null) {
						GameObject.Find("GameController").GetComponent<GameController>().LeftClick(objectName, hit.transform.position);
					}
				...
				if(Physics.Raycast (ray, out hit, 200.0f) && objectName != null) {
					GameObject.Find("GameController").GetComponent<GameController>().RightClick(objectName, hit.transform.name, hit.transform.position.x, hit.transform.position.z);
				}

				GameObject.Find ("GameController").GetComponent<GameController> ().isActionButton = false;
			}
		}
